...
> ملاحظة: تم تحديث السكربت ليشمل جداول إدارة الأدوار: `roles`, `pages`, `actions`، مع تريغر يضمن إصلاح الترميز تلقائياً عند الإدراج/التحديث، بالإضافة إلى تحديث البيانات القديمة.

### 2. إصلاح الواجهة الأمامية لقراءة البيانات القديمة
تمت إضافة أداة تنظيف للترميز في `src/utils/encoding.ts` وتطبيقها في `src/services/roles.service.ts` بحيث يتم تصحيح أي Mojibake عند القراءة (مثل "Ù…Ø¯ÙŠØ± Ø§Ù„Ù†Ø¸Ø§Ù…"). لا حاجة لتدخل يدوي في الواجهة.
